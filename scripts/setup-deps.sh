...
if command -v apt-get >/dev/null 2>&1; then
	...
	sudo apt-get install -y \
		clang llvm \
		libelf-dev zlib1g-dev libssl-dev libyaml-dev libzstd-dev \
		make pkg-config \
		gcc
	...
	sudo dnf install -y clang llvm elfutils-libelf-devel zlib-devel openssl-devel libyaml-devel libzstd-devel make gcc pkgconf
elif command -v pacman >/dev/null 2>&1; then
	sudo pacman -S --needed clang llvm libelf zlib openssl libyaml make gcc pkgconf
else
	echo "Unsupported package manager; install: clang llvm libelf-dev zlib-dev libssl-dev libyaml-dev libzstd-dev make gcc" >&2
	exit 1
fi
